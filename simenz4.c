/* 
   ...
   *
   * Glicolisis en E. Histolytica -> Secuencia de reacciones verificada
   *
   * Mecanismo para enzimas bireactantes y terreactantes revisado
   *
   * Secuencia de reacciones revisada de nuevo (17/07/2008)
   * 
   * Se revisa todo de nuevo (13/08/2008)
   *  
...
#define NSUB 21		// Numero de substancias
...
#define TTOP 10.0		// tiempo de simulacion, en segundos
...
main()
{
   ...
   double mm[NREAC] = {0.0};
   int n0,n1,n2,n3,n4,n5,n6,n7,n8,n9,n10,n11,n12,n13,n14,n15,n16,n17,n18,n19,n20,TOP,lineas=0;
   ...
   //****************************** Ajuste del numero de puntos a escribir *****************************************
   ...
   */
   ...
   km[4][12]=455;	// Fru6P
   km[5][7]=1440.0;	// Pi
   km[5][13]=124.0;     // Fru(1-6)P2
   
   // ALDO
   km[6][13]=4.0;	// Fru(1-6)P2
   km[7][14]=105.0;	// GrnP
   km[7][15]=108.0;	// Galh3P

   // TPI
   km[8][14]=1400.0; 	// GrnP
   km[9][15]=740.0;	// Galh3P

   // GAPDH
   km[10][8]=59.0;	// NADox
   km[10][15]=33.0;	// Galh3P
   km[11][9]= 118.0;	// NADred (ASUMIDO)
   km[11][16]=10.0;	// 1,3BPG
   km[11][17]=246.0;	// 3PG
   
   // PGK
   km[12][2]=3400;	// ADP
   km[12][5]=292.0;	// GDP
   km[12][16]=127.0;	// 1,3BPG
   km[13][1]=3300.0;	// ATP
   km[13][4]=75.0;	// GTP
   km[13][17]=570.0;	// 3PG
   ...
   km[18][6]=470.0;	// PPi
   km[18][19]=24.0;	// PPP
   km[19][1]=40.0;	// ATP (ASUMIDO)
   km[19][20]=68.0;	// Pyr 

   // Las afinidades se expresan en numero de sustratos (n_M)
   for(j=0;j<2*NREAC;j++)
   {
      for(k=0; k<NSUB; k++)	   
      {	      
   ...
   fprintf(dptr,"%lf \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf \n",tiempo,(double)n[0]*1.e6/(NA*VSIM*1.e3),(double)n[1]*1.e6/(NA*VSIM*1.e3),(double)n[2]*1.e6/(NA*VSIM*1.e3),(double)n[3]*1.e6/(NA*VSIM*1.e3),(double)n[4]*1.e6/(NA*VSIM*1.e3),(double)n[5]*1.e6/(NA*VSIM*1.e3),(double)n[6]*1.e6/(NA*VSIM*1.e3),(double)n[7]*1.e6/(NA*VSIM*1.e3),(double)n[8]*1.e6/(NA*VSIM*1.e3),(double)n[9]*1.e6/(NA*VSIM*1.e3),(double)n[10]*1.e6/(NA*VSIM*1.e3),(double)n[11]*1.e6/(NA*VSIM*1.e3),(double)n[12]*1.e6/(NA*VSIM*1.e3),(double)n[13]*1.e6/(NA*VSIM*1.e3),(double)n[14]*1.e6/(NA*VSIM*1.e3),(double)n[15]*1.e6/(NA*VSIM*1.e3),(double)n[16]*1.e6/(NA*VSIM*1.e3),(double)n[17]*1.e6/(NA*VSIM*1.e3),(double)n[18]*1.e6/(NA*VSIM*1.e3),(double)n[19]*1.e6/(NA*VSIM*1.e3),(double)n[20]*1.e6/(NA*VSIM*1.e3) ); 
...
void probab()
{
   ...
   int j,k,cdir,cinv,see;
   double taub[2][NREAC]={0.0};
   double pimp;
   ...
   for(j=0;j<NREAC;j++)
   {
      for(k=0;k<NSUB;k++)			// Recorre las substancias de una reaccion
      { 
         if(eps[j][k]<0) 			// Reaccion directa
         {
            if(abs((eps[j][k])<10)&&(n[k]!=0))		 
	       taub[0][j]*=1.+km[2*j][k]/(double)n[k];	    
	    else if((abs(eps[j][k])>=10)&&(n[k]!=0))		 
	       taub[0][j]*=1.+km[2*j][k]/(double)n[k];	    
	    else if(n[k]==0)
	       taub[0][j]=10.e20;	    
	    pimp = km[2*j][k];
	    ...
	    pimp = taub[0][j];
         ...
         {
            if(km[2*j+1][k]!=0.0)		// Substancia participa en la reaccion
	    {	       
               if((abs(eps[j][k])<10)&&(n[k]!=0))		 
	          taub[1][j]*=1.+km[2*j+1][k]/(double)n[k];	    
	       else if((abs(eps[j][k])>=10)&&(n[k]!=0))		 
	          taub[j][j]*=1.+km[2*j+1][k]/(double)n[k];	    
	       else if(n[k]==0)
	          taub[1][j]=10.e20;	    
	    } 	  
         }   
...
int pos1,pos2,pos3;
{
   ...
   // extern int ***estado;
   // extern int **act;
   double state_out=0.0,dado,umbral,pimp_d;
   int gof=0,gob=0,state,pimp_i;
   ...
   if(abs(pos3)>50)
      pimp_i=0;
   pimp_i=estado[pos1][pos2][pos3];
   pimp_d=1.0+bdir[pos1];
...
int pos1,pos2,pos3;
{
   ...
   if(estado[pos1][pos2][pos3]==1)			
   {
      if((state_in<=1.0)&(state_out>1.0)&(gof==1))	
      {
         ...
         { 
            ...
            {		    
               if(abs(eps[pos1][m])<10)   		    
                  n[m]+=eps[pos1][m];		// Actualiza concentraciones
               if(abs(eps[pos1][m])>=10)   		    
                  n[m]+=eps[pos1][m]%10;	// Actualiza concentraciones
	    }    
         }
   ...
   if(estado[pos1][pos2][pos3]==2)
   {
      ...
      if(state_out>=phicdir[pos1])	
      {
         for(j=0;j<NSUB;j++)
         {
            //if((eps[pos1][j]>0)&&(eps[pos1][j]!=20))	// Libera producto
            if((eps[pos1][j]>0)&&(eps[pos1][j]<10))	// Libera producto
   ...
   if(estado[pos1][pos2][pos3]==-2)
   {
      ...
      if(state_out>=phicinv[pos1])	
      {
         ...
         {
            //if((eps[pos1][j]<0)&&(eps[pos1][j]!=-20))		 		// Libera producto
            if((eps[pos1][j]<0)&&(abs(eps[pos1][j])<10))			// Libera producto
               n[j]-=eps[pos1][j];			// Actualiza concentracion
         }
...
int pos1;
{
   ...
   for(m=0;m<NSUB;m++)
   {
	 ...
	 if(abs(eps[pos1][m])>=10)     
         {		 
            if(n[m]>=abs(eps[pos1][m])%10) 	// Si TODOS los sustratos estan disponibles...
               gof*=1;				// ...la reaccion procede
            else
               gof*=0;                        
	 }   
...
int pos1;
{
   ...
   for(m=0;m<NSUB;m++)
   {
      if(eps[pos1][m]>0)			// Si sustancia es sustrato de la reaccion inversa...
      {
	    ...
	    if(abs(eps[pos1][m])>=10)     
            {		    
               if(n[m]>=eps[pos1][m]%10) 		// Si TODOS los sustratos estan disponibles...
               {		    
                  gob*=1;				// ...la reaccion procede
	          flag=0;
	       }   
