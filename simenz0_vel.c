...
#define NENZ 20		// Numero de enzimas
#define INH 1.7734		// Concentracion de inhibidor, en micromolar
#define KI 1.7734		// Afinidad por el inhibidor, en micromolar
...
#define ATOP 1000.0 	// Concentracion final de sustrato (microMolar)
#define NTOP 300000     // Numero de iteraciones para el calculo de la velocidad
//#define NTOP 100000     // Numero de iteraciones para el calculo de la velocidad
#define AVG  1 		// Numero de realizaciones
...
main()
{
   ...
   double prods,B,prodp,tiempo=0.0,deltat,dado,tau,deltaA,vel=0.0,cA,vel_prom,v_teor,vmax0;
   double mm[NREAC] = {0.0},pimp,kmap,km0;
   ...
   */
   ...
   printf("\nEl numero de inhibidores es %d",I);
   pimp=I;
   ...
   inh[0][2] = 10;		// Substancia inhibidora, reaccion directa, inc
   ...
   // Calcula afinidad discreta para inhibidores
   /*for(k=0;k<NSUB;k++)	   
   {	      
      ki[k]*=NA*1.e-6*VSIM;	      
      if(ki[k] != 0.0)
         printf("\nks[%d] es %lf [sustratos]",k,ki[k]);
   }*/	 
   ...
   while(flag == 0)
   {
	 ...
	 if(n[0]>3099)
	    ...
	    // Toma en cuenta solo el producto en el ultimo 25% de las iteraciones
	    /*if((zz>0.75*NTOP)&&(flag1==0))
	    {	    
	       n[1]=0;	    
	       flag1=1;
	    }*/  
	 ...
	 // Calcula velocidad
	 vel=n[1]*1.e6*60./(NA*VSIM*tiempo);
	 vel_prom+=vel;
      ...
      //vel_prom=1.0*vel_prom/(float)AVG;
      vel_prom=1.0*vel_prom/(float)AVG;

      //v_teor=1.03*vmax0*(NENZ*mm[0]*1.e3/(NA*VSIM))*cA/(cA+km0);
      v_teor=1.0*vmax0*(NENZ*mm[0]*1.e3/(NA*VSIM))*cA/(cA+km0);
      
      // Registra concentraciones de sustancias
      fprintf(dptr,"%lf \t %lf \t %lf\t %lf \n",cA,vel_prom,v_teor,0.5*v_teor); 

      printf("\nLa concentracion es %lf, equivalente a %d sustratos, salieron %d sustratos, vel= %lf, I = %d",cA,(int)A_acc,n[1],vel_prom,I);
      cA+=deltaA;
      A_acc+=deltaS;	
...
int pos1,pos2,pos3;
{
   ...
   if(inh[pos1][0]!=-1) 			// R. directa, I. competitiva
   {
	 ...
	 pimp=Ki;
	 pimp=vmax[pos1][0];
         pin=DT*0.5*I*vmax[pos1][0]/Ki;	
         pin=10.0*DT*I*vmax[pos1][0]/Ki;	
         pout=.02; 
      } 
   }
   ...
   else if(inh[pos1][2]!=-1) 			// R. directa, I. no competitiva
   {
      ...
      { 
         ...
         //pin=2500.0*DT*(double)I*vmax[pos1][0]/Ki;	
         pin=350000.0*DT*(double)I/Ki;
         pout=0.022;
	 //printf("\nI = %d",I);
	 //printf("\npin = %lf",pin);
      }
   ...
   if((inh[pos1][2]!=-1)||(inh[pos1][3]!=-1))
   {
      ...
      else if((gof==1)&(estado[pos1][pos2][pos3]==5))  
      {
         ...
         if(state_out>1.0)
         {
            ...
            //if(estado[pos1][pos2][pos3-1]==2)		// Enzima va hacia adelante
