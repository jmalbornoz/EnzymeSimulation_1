...
#define VSIM 10.3e-20 	// Volumen de simulacion (litros)
#define NINJ 200.0 	// Concentracion final de sustrato (microMolar)
#define TTOP 5.0		// tiempo de simulacion, en segundos
   ...
   for(j=0;j<NREAC;j++)
   {
      ...
      { 
         if(eps[j][k]<0) 			// Reaccion directa
         {
            ...
            if(abs(eps[j][k])>=10)		 
               p1[0][j]*=DT*pow((double)n[k]*(vmax[j][0]/km[2*j][k]),(double)(abs(eps[j][k])%10));
	    yo = n[k];
	    if(n[k] == 1)
	       yo = 0;	    
	    pimp = p1[0][j];
	    pimp = km[2*j][k];
         ...
         if(eps[j][k]>0) 			// Reaccion inversa
         {
	       ...
	       {	       
                  ...
                  if(abs(eps[j][k])>=10)		 
                     p1[1][j]*=DT*pow((double)n[k]*(vmax[j][1]/km[2*j+1][k]),(double)(abs(eps[j][k])%10));   
	       } 	  
            } 
            if(p1[1][j]>=1)
               p1[1][j]=0.99999999;   
